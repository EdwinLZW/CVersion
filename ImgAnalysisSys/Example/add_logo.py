...
from BasicModel.BasicOperation import *
...
class AddLogoToImage(object):
    ...
    def addlogo(self, bgimg, logoimg, *args):
        if len(args) == 4:
            ...
            roi = __bg_img[__bg_img.shape[0] - rows:__bg_img.shape[0], __bg_img.shape[1] - clos:__bg_img.shape[1]]
            ...
            cv2.imshow('addlogo', cv2.resize(__bg_img, (200,200)))
            cv2.waitKey(0)
        else:
